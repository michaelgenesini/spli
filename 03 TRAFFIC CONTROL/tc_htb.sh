...
iptables -A OUTPUT -t mangle -d $marci -p tcp --sport 80 -j MARK --set-mark 40
iptables -A OUTPUT -t mangle -p tcp --sport 22 -j MARK --set-mark 30
...
tc class add dev $INTIF parent 1:1 classid 1:10 htb rate 100Mbps
...
tc class add dev $INTIF parent 1:1 classid 1:13 htb rate 100kbps
tc class add dev $INTIF parent 1:1 classid 1:14 htb rate 100kbps ceil 250kbps prio 1
tc class add dev $INTIF parent 1:1 classid 1:15 htb rate 20kbps ceil 80kbps prio 0

#Arrivato qui ho due modi per applicare le regole (perdita, delay, bit-rate associata ecc..) NETEM O TBF:

tc qdisc add dev $INTIF parent 1:10 handle 63: sfq
tc qdisc add dev $INTIF parent 1:11 handle 10: netem delay 100ms 10ms
tc qdisc add dev $INTIF parent 1:12 handle 20: netem corrupt 50%
tc qdisc add dev $INTIF parent 1:13 handle 60: tbf rate 20kbps buffer 1600 limit 3000
tc qdisc add dev $INTIF parent 1:14 handle 61: sfq
tc qdisc add dev $INTIF parent 1:15 handle 62: sfq
...
#Filtro su un IP destinatario per corrupt
tc filter add dev $INTIF protocol ip parent 1:0 prio 3 u32 match ip dst $mikeubu flowid 1:12

#Filtro su un IP destinatario per il delay
tc filter add dev $INTIF protocol ip parent 1:0 prio 3 u32 match ip dst $marco flowid 1:11

#Filtro per IP per tbf
tc filter add dev $INTIF protocol ip parent 1:0 prio 2 u32 match ip dst $pedro flowid 1:13

tc filter add dev $INTIF protocol ip parent 1:0 prio 2 u32 match ip dst $mike flowid 1:10
